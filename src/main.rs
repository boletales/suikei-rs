...
fn get_images(zoom:i32,lat:f64,long:f64,size:i32,pow:f64,mag:f64) -> Images {
  ...
  let pretty_count : Vec<Vec<f64>> = count.iter().map(|v| v.iter().map(|c| f64::powf(*c as f64,pow)).collect()).collect();
  ...
  let river = ImageBuffer::from_fn(w as u32, h as u32, |x, y| {
    let l = f64::floor((f64::min(1.0,pretty_count[y as usize][x as usize] - min_pcnt) / (max_pcnt/mag - min_pcnt)) * 255.0) as u8;
    image::Rgb([l, l, l])
  });

  let colored = ImageBuffer::from_fn(w as u32, h as u32, |x, y| {
    let l = f64_clip((pretty_count[y as usize][x as usize] - min_pcnt) / (max_pcnt/mag - min_pcnt),0.0,1.0);
